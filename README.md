...
This library is a wrapper for the [Google Maps JavaScript APIv3](https://developers.google.com/maps/documentation/javascript/tutorial). It allowed us to quickly create components for our map, but we eventually ran into limitations that were not described in the user documentation. A few of those challenges and solutions are described below.

### Creating reports from clicked points on the map
We were inspired by the simplicity of the Waze app and wanted ours to help users to quickly report what's in stock without spending a ton of time filling out details on a form.

Our initial approach was to geocode a marker on the map wherever a user clicked on an open map space. Clicking would assign the `selectedCoords` slice of the `Map` component’s state to a set of latitude/longitude coordinates.  
...
```
The `selectedCoords` were then passed to the `ReportFormContainer` as props where a user would enter a custom name for the report. 

While this worked, it limited our reports in two important ways:
...
In the `Map` component, add a slice of state for `selectedVendor` and a `selectedReport` that represents the Google Places object returned by the `SearchBox` component.
...
```
After iterating through reports, identify whether a new `ReportFormContainer` should be created at the coordinates of the `place`. This was the trickiest logic because we needed to assert that both  of the following were true: + there was no matching report earlier in the function + there was no matching report in a previous search (which would still be stored in the `Map` state. Additionally, we wanted to ensure only authenticated users could submit reports. 
```
...
While it would have been easiest to only pass the Google Maps Places `place` object to the `ReportFormContainer`, we ultimately needed to pass each Place Details that we wanted as its own prop because the Google Maps API was blocking us from accessing Place Details from a component where the Google Maps API Key was not defined.
...
These props are stored in the state of the `ReportFormContainer` in order to give the user the flexibility to alter some, but not all details. The main one that needs to stay consistent is the placeId.
